...
CC_INCLUDE = -I$(PROJ_DIR)/ \
			 -I$(PROJ_DIR)/include/ \
			 -I$(PROJ_DIR)/driver/ \
			 -I$(PROJ_DIR)/logic/ \
			 -I$(PROJ_DIR)/bluerobin/ \
			 -I$(PROJ_DIR)/simpliciti/ \
			 -I$(PROJ_DIR)/simpliciti/Components/bsp \
			 -I$(PROJ_DIR)/simpliciti/Components/bsp/drivers \
			 -I$(PROJ_DIR)/simpliciti/Components/bsp/boards/CC430EM \
			 -I$(PROJ_DIR)/simpliciti/Components/mrfi \
			 -I$(PROJ_DIR)/simpliciti/Components/nwk \
			 -I$(PROJ_DIR)/simpliciti/Components/nwk_applications

CC_COPT		=  $(VERBOSE) $(CC_CMACH) $(CC_DMACH) $(CC_DOPT)  $(CC_INCLUDE) 

LOGIC_SOURCE = logic/acceleration.c \
			   logic/alarm.c \
			   logic/altitude.c \
			   logic/battery.c  \
			   logic/clock.c \
...
SIMPLICICTI_SOURCE = $(SIMPLICICTI_SOURCE_ODD) \
...
ALL_S = $(addsuffix .s,$(basename $(LOGIC_SOURCE))) \
		$(addsuffix .s,$(basename $(DRIVER_SOURCE))) \
		$(addsuffix .s,$(basename $(SIMPLICICTI_SOURCE)))  \
        $(addsuffix .s,$(basename $(MAIN_SOURCE))) \
		#

EXTRA_O = $(OBJ_DIR)/even_in_range.o 
ALL_O = $(patsubst %.c,$(OBJ_DIR)/%.o,$(addsuffix .c, $(basename $(ALL_C))))
...
main: build config.h even_in_range $(ALL_O) $(EXTRA_O) build
	...
	$(PYTHON) tools/memory.py -i $(BUILD_DIR)/eZChronos.elf -o $(BUILD_DIR)/eZChronos.txt

$(ALL_O): $(OBJ_DIR)/%.o: %.c config.h include/project.h
	@mkdir -p $(dir $@)
...
debug:	build even_in_range $(ALL_O)
	...
	@echo "Convert to TI Hex file"
	$(PYTHON) tools/memory.py -i $(BUILD_DIR)/eZChronos.dbg.elf -o $(BUILD_DIR)/eZChronos.txt
...
even_in_range:
	...
	@mkdir -p $(OBJ_DIR)
	$(CC) -D_GNU_ASSEMBLER_ -x assembler-with-cpp -c even_in_range.s -o $(OBJ_DIR)/even_in_range.o
